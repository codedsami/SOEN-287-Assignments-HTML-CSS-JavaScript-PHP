  ...
  dateElement.textContent = formatDate(now);
        ...
        function validateGiveAwayForm(){
          ...
          if(breed === '') {
           ...
           if(name.value.trim() === '') {
            alert('Please enter the name of the pet owner');
            return false;
           }
...
if (window.location.href.includes('Find_a_dog_cat_form.html')) {
  ...
  catOptions.forEach(option => {
    if (option.checked) {
      option.checked = false;
			...
			// function addInterested(name) {
				// 	let interestedList = document.querySelector('#interested ul');
				// 	let listItem = document.createElement('li');
				// 	let link = document.createElement('a');
				// 	link.href = '#' + name;
				// 	link.textContent = name;
				// 	listItem.appendChild(link);
				// 	interestedList.appendChild(listItem);
				// }